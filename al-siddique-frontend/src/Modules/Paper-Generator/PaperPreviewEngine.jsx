...
function LogoWatermark({ logo, show, opacity = 0.08 }) {
...
function ClassicTemplate({ config, selectedMCQ, selectedShort, selectedLong, settings, showAnswers, edit, printOpts, selectedChapters=[] }) {
 ...
 const bodyFont = isUrdu ? URDU_FONT : 'Times New Roman, Times, serif'
 ...
 {/* Info Table */}
 <table style={{ width:'100%', borderCollapse:'collapse', border:'1.5px solid #111', marginBottom:8 }}>
 <tbody>
 <tr>
 <td rowSpan={2} style={{ width:70, border:'1.5px solid #111', textAlign:'center', verticalAlign:'middle', padding:5 }}>
 {settings.logo
 ? <img src={settings.logo} style={{ width:56, height:56, objectFit:'contain' }} alt="logo" />
 : <div style={{ width:56, height:56, borderRadius:'50%', border:'2px solid #1a3a6b', display:'flex', alignItems:'center', justifyContent:'center', margin:'auto', fontSize:7, fontWeight:800, color:'#1a3a6b', textAlign:'center', lineHeight:1.2, fontFamily:'Arial,sans-serif' }}>AL<br/>SIDDIQUE</div>
 }
 </td>
 ...
 <AnsLines count={printOpts.blankLines||2} color='#bbb' mt={14} />
 </div>
 ))}
 </div>
 </div>
 ...
 <AnsLines count={printOpts.longLines||7} color='#ccc' mt={18} />
 </div>
 ))}
 </div>
 )})()}
...
function ModernTemplate({ config, selectedMCQ, selectedShort, selectedLong, settings, showAnswers, edit, printOpts, selectedChapters=[] }) {
 ...
 return (
 <div className="ppe-paper" style={{ position:'relative', background:'#fff', color:'#1a1a2e', fontFamily:isUrdu?URDU_FONT:`'Segoe UI',Arial,sans-serif`, fontSize:12, lineHeight:lineH, maxWidth:794, margin:'0 auto', padding:'12px 16px', direction:isUrdu?'rtl':'ltr' }}>
 <LogoWatermark logo={settings.logo} show={printOpts.showLogoWatermark} opacity={printOpts.watermarkOpacity} />
 ...
 {settings.logo
 ? <img src={settings.logo} style={{ width:58, height:58, objectFit:'contain', borderRadius:0, background:'#fff', padding:4, flexShrink:0 }} alt="logo" />
 : <div style={{ width:58, height:58, borderRadius:'50%', border:'2px solid rgba(255,255,255,0.5)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, fontWeight:800, flexShrink:0 }}>AS</div>
 }
 <div style={{ flex:1 }}>
 ...
 {(config.language==='urdu'||config.language==='mixed') && q.textUrdu && <div style={{ fontFamily:URDU_FONT, direction:'rtl', fontSize:urdFS }}>{q.textUrdu}</div>}
 {showAnswers&&q.answer && <div style={{ color:'#27ae60', fontSize:10 }}>{q.answer}</div>}
 <AnsLines count={printOpts.blankLines||2} color='#ccc' mt={14} />
 </div>
 ))}
 </div>
 </div>
 ...
 {(config.language==='urdu'||config.language==='mixed') && q.textUrdu && <div style={{ fontFamily:URDU_FONT, direction:'rtl', fontSize:urdFS }}>{q.textUrdu}</div>}
 {showAnswers&&q.answer && <div style={{ color:'#27ae60', fontSize:10 }}>{q.answer}</div>}
 <AnsLines count={printOpts.longLines||7} color='#ddd' mt={20} />
 </div>
 ))}
 </div>
 </div>
...
function EliteTemplate({ config, selectedMCQ, selectedShort, selectedLong, settings, showAnswers, edit, printOpts, selectedChapters=[] }) {
 ...
 {settings.logo
 ? <img src={settings.logo} style={{ width:72, height:72, objectFit:'contain', borderRadius:0, background:'#fff', padding:4, flexShrink:0 }} alt="logo" />
 : <div style={{ width:72, height:72, borderRadius:'50%', border:`3px solid ${gold}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, fontWeight:800, color:gold, flexShrink:0 }}>AS</div>
 }
 <div style={{ flex:1 }}>
 ...
 <table style={{ width:'100%', borderCollapse:'collapse', border:`1.5px solid ${gold}`, marginBottom:12 }}>
 <tbody>
 <tr>
 ...
 {(config.language==='urdu'||config.language==='mixed') && q.textUrdu && <div style={{ fontFamily:URDU_FONT, direction:'rtl', fontSize:urdFS }}>{q.textUrdu}</div>}
 {showAnswers&&q.answer && <div style={{ color:'#27ae60', fontSize:10 }}>{q.answer}</div>}
 <AnsLines count={printOpts.blankLines||2} color='#ddd' mt={16} />
 </div>
 ))}
 </div>
 </div>
 ...
 {(config.language==='urdu'||config.language==='mixed') && q.textUrdu && <div style={{ fontFamily:URDU_FONT, direction:'rtl', fontSize:urdFS }}>{q.textUrdu}</div>}
 {showAnswers&&q.answer && <div style={{ color:'#27ae60', fontSize:10 }}>{q.answer}</div>}
 <AnsLines count={printOpts.longLines||7} color='#eee' mt={20} />
 </div>
 ))}
 </div>
 </div>
...
export default function PaperPreviewEngine({ config, selectedMCQ, selectedShort, selectedLong, settings, showAnswers: initAns, onBack, selectedChapters=[], importToQuestionBank = false, questionBankSubjectId = '', questionBankSubjectMeta = null, paperSource = 'paper' }) {
 ...
 <NumOpt label="Long" value={printOpts.longLines} min={2} max={15} onChange={v=>setPO('longLines',v)} />
 </div>
 </div>
