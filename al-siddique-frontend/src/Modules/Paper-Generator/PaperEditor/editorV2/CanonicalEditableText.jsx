// CanonicalEditableText.jsx — Focus-Stable In-Place Tiptap Field Editor (Rules 28, 29, 30)
...
export default function CanonicalEditableText({
  fieldKey,
  fieldOverlay,
  direction = 'auto',
  store,
...
function ActiveInPlaceEditor({
  ...
  const editor = useEditor({
    ...
    onFocus: ({ editor: activeEd }) => {
      registry?.setActiveFieldKey(fieldKey)
      onFocusField?.(fieldKey)
    },
    onSelectionUpdate: ({ editor: activeEd }) => {
      registry?.setActiveFieldKey(fieldKey)
    },
    onUpdate: ({ editor: updatedEd }) => {
      isLocalUpdateRef.current = true
      // Local typing updates compact working store directly (Rule 28)
      // Does NOT trigger full React document rerender or clonePaperDocument
      store?.updateField(fieldKey, updatedEd.getJSON(), updatedEd.getText())
      setTimeout(() => {
